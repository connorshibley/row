...
//
// NOTE: the Supabase config below is the REAL project URL + publishable
// key, and the water +1 push is auth-aware (per-user RLS): it reuses
// auth.js's signed-in client, includes user_id, and upserts on
// (user_id, key). Do NOT downgrade this to an anon placeholder model —
// the app_state table is private per user.
// =============================================================
(function () {
  'use strict';

  // -------- Supabase config (same project as the rest of the dashboard) --------
  const TOPBAR_SUPABASE_URL = 'https://ygeeeplqpudlodoquwly.supabase.co';
  const TOPBAR_SUPABASE_KEY = 'sb_publishable_ldoSboZs6XCL7fOBbVA0-g_ZzLk2O4L';
...
.topbar {
  ...
  gap: 8px;
  padding: max(10px, env(safe-area-inset-top)) calc(14px + env(safe-area-inset-right)) 8px calc(14px + env(safe-area-inset-left));
  background: #0a0a0b;
...
<nav class="bottombar" id="bottombar" role="navigation" aria-label="Main tabs">
  ...
  // Auth-aware water push: reuse auth.js's signed-in client, scope the row to
  // the current user, and upsert on (user_id, key). Runs on every page (the
  // health page included) because cloud-sync.js has no cross-frame listener to
  // catch a +1 logged from the parent while the tracker is embedded.
  async function pushWaterMergedToSupabase(localWater) {
    ...
    try {
      ...
      if (!session) return; // signed out — auth.js shows the login gate
    ...
    } catch (e) { /* offline or signed out — local change syncs on next visit */ }
  }
  function addWater() {
    let state = null;
    try { state = JSON.parse(localStorage.getItem('po_water_v1')); } catch (e) {}
